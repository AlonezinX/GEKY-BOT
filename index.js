...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:GEKYUME GOSTOSO\n' 
            + 'ORG:Dono do GEKY-BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5511934146180:+55 (11) 93414-6180\n' 
            + 'END:VCARD'
...
async function starts() {
	...
	tiringa.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o GEKY-BOT!'))
	})

	fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
	tiringa.on('connecting', () => {
		start('2', 'Conectando o GENKY-BOT...')
	})
	tiringa.on('open', () => {
		success('2', 'GEKY-BOT conectado!!!')
	})
...
tiringa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]} vai pela sombra pq bosta no sol seca👋😂`
    ...
    tiringa.on('CB:Blocklist', json => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`5511934146180@s.whatsapp.net`]
			const ownerNumberB = []
...
//_TIPO DE USUÁRIO
if (groupAdmins) {prema = 'Ademir🤓'}if (isPremium) {prema = 'Premium😎'} if (isOwner) {prema = 'El GEKYUME🕴🏽'} if (isUser) {prema = 'Registrado 🥳'} else {var prema = 'Membro comum🗿'}
...
tiringa.sendMessage('5511934146180@s.whatsapp.net', options, text, {quoted: mek})
...
case 'pão':
if (isOwner) return reply('Oi GEKYUME')
else {
...
case 'GEKYUME':
case 'italo':
await tiringa.sendMessage(from, {displayname: "GEKYUME🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('BOT-GEKY', 'GEKYUME')} ${ran} -o ${ran}`, async (error) => {
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('GEKY-BOT', 'GEKYUME')} ${ran} -o ${ran}`, async (error) => {
if (error) return reply(ptbr.stick())
tiringa.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('GEKY-BOT', 'GEKYUME')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('GEKY-BOT', 'GEKYUME')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
case 'clearall':
if (!isOwner) return reply('Só o GEKYUME pode fazer isso')
...
if (!q.includes('|')) return reply(`Digite da forma correta:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar GEKYUME|18`)
...
if (vit == "derrota" ) {
var tes = "A vitória é do GEKY-BOT"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`GEKY-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
tiringa.sendMessage(from, `🎉🎉 VITÓRIA DO GEKY-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
} else {
tiringa.sendMessage(from, `🎉🎉 VITÓRIA DO GEKY-BOT 🎉🎉`, text)
}
